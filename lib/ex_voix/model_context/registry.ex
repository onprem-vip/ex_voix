    ...
    cond do
      is_map(tools) ->
        if exact_name do
          ...
          [Enum.filter(tools |> Map.keys(), fn t -> not is_nil(t) end) |> Enum.max_by(fn t -> String.jaro_distance(name, t) * 100 end)]
        end
      ...
      true ->
        if exact_name do
          ...
          [Enum.filter(tools, fn t -> not is_nil(Map.get(t, "name")) end) |> Enum.max_by(fn t -> String.jaro_distance(name, Map.get(t, "name")) * 100 end)]
        end
