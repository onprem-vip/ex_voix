...
const voixDefaultResourceHandler = (resource, element) => {
    if (resource && (
...
const VoixEventHandler = {
    init(commandResourceHandler = null, defaultResourceHandler = null) {
        window.voixHandler = null;
        if (commandResourceHandler)
            window.commandResourceHandler = commandResourceHandler;
        else
            window.commandResourceHandler = voixCommandResourceHandler;
        if (defaultResourceHandler)
            window.defaultResourceHandler = defaultResourceHandler;
        else
            window.defaultResourceHandler = voixDefaultResourceHandler;
    },
    mounted() {
